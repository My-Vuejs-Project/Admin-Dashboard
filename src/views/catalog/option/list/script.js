...
export default {
  ...
  methods: {
    ...
    getOptions() {
      ...
      Service.getAllOption(param).then((response) => {
    ...
    deleteOption() {
      ...
      let optionId = this.data.options[this.fields.index].id;
      Service.deleteOption(optionId).then((response) => {
        if (response.statusCode) {
          if (response.statusCode == "4410") {
            Service.refreshToken().then((response) => {
              if (response == "ok") {
          ...
          this.isDeleting = false;
          this.data.options.splice(this.fields.index, 1);
          this.data.pagination.total -= 1;
          this.$toasted.show("Option has been deleted.");
          this.closeModal();
        }
      });
    },
