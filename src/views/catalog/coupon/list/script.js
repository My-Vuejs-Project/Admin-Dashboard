...
export default {
  ...
  methods: {
    ...
    getCoupons() {
      ...
      Service.getAllCoupons(param).then((response) => {
        if (response.statusCode) {
          ...
          if (response.data.length > 0) {
            this.isNoData = false;
            this.data.coupons = response.data;
    ...
    deleteCoupon() {
      ...
      let couponId = this.data.coupons[this.fields.index].id;

      Service.deleteCoupon(couponId).then((response) => {
        if (response.statusCode) {
          if (response.statusCode == "4410") {
            Service.refreshToken().then((response) => {
              if (response == "ok") {
          ...
          this.isDeleting = false;
          this.data.coupons.splice(this.fields.index, 1);
          this.data.pagination.total -= 1;
          this.$toasted.show("Coupon has been deleted.");
          this.closeModal();
        }
      });
    },
