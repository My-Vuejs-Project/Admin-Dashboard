...
export default {
  ...
  methods: {
    ...
    getAllManufacturers() {
      ...
      Service.getAllManufacturers(param).then((response) => {
        if (response.statusCode) {
          ...
          if (response.data.length > 0) {
            this.isNoData = false;
            this.data.manufacturers = response.data;
    ...
    deleteManufacturer() {
      ...
      let manufacturerId = this.data.manufacturers[this.fields.index].id;
      Service.deleteManufacturer(manufacturerId).then((response) => {
        if (response.statusCode) {
          if (response.statusCode == "4410") {
            Service.refreshToken().then((response) => {
              if (response == "ok") {
          ...
          this.isDeleting = false;
          this.data.manufacturers.splice(this.fields.index, 1);
          this.data.pagination.total -= 1;
          this.$toasted.show("Manufacturer has been deleted.");
          this.closeModal();
        }
      });
    },
